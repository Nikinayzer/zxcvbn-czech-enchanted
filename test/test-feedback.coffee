...
test 'custom feedback messages', (t) ->
  ...
  # If suggestion is present in custom messages and has a falsy value returns empty array
  custom_messages =
    top10_common_password: 'custom#top10_common_password',
    uncommon_words_are_better: null
  f = feedback.get_feedback(1, [match], custom_messages)
  t.equal f.warning, custom_messages.top10_common_password
  t.deepEqual f.suggestions, []

  # If one of suggestions is present in custom messages and has a falsy value returns only others suggestion(s)
  f = feedback.get_feedback(1, [Object.assign {}, match, {token: 'Capitalizedtoken', rank: 10}], custom_messages)
  t.equal f.warning, custom_messages.top10_common_password
  t.deepEqual f.suggestions, [feedback.messages.capitalization_doesnt_help]

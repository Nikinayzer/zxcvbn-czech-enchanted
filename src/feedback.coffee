...
feedback_l10n = require('./feedback_l10n')

feedback =
  get_feedback: (score, sequence, custom_messages, language = "en") ->
    @custom_messages = custom_messages

    @messages = feedback_l10n.en
    if (language && language of feedback_l10n)
      @messages = feedback_l10n[language]
  ...
  get_dictionary_match_feedback: (match, is_sole_match) ->
    warning = if match.dictionary_name.indexOf('passwords') != -1
      if is_sole_match and not match.l33t and not match.reversed
      #if is_sole_match and match.token == match.matched_word
        if match.rank <= 10
    ...
    else if match.dictionary_name.indexOf('names') != -1
      if is_sole_match
        'names_are_easy'
      else
